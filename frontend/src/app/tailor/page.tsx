              ...
              {generated && (
                <div style={{ background: BRAND.surface, borderRadius: "12px", border: `1px solid ${BRAND.border}`, padding: "1.25rem" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                    {[
                      { label: "Tailor assets", done: true },
                      { label: "Download resume & cover letter", done: false },
                      { label: "Apply on website", done: false },
                    ].map((step) => (
                      <div key={step.label} style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                        <div style={{ width: 18, height: 18, borderRadius: "50%", border: `1.5px solid ${step.done ? BRAND.green : BRAND.border}`, background: step.done ? BRAND.greenBg : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                          {step.done && <span style={{ color: BRAND.green, fontSize: "0.65rem", fontWeight: 700 }}>✓</span>}
                        </div>
                        <span style={{ fontSize: "0.8rem", color: step.done ? BRAND.green : BRAND.muted, fontWeight: step.done ? 500 : 400 }}>{step.label}</span>
                      </div>
                    ))}
                  </div>
                  {job?.source_url && (
                    <a href={job.source_url} target="_blank" rel="noopener noreferrer" style={{ display: "block", marginTop: "1rem", padding: "0.7rem", borderRadius: "8px", background: BRAND.green, color: "#fff", textAlign: "center", fontSize: "0.875rem", fontWeight: 600, textDecoration: "none" }}>
